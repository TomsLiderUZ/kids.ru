        ...
        <>
            ...
            <div className={`${styles.alert} ${wrong ? styles.wrong : undefined}`}>
                <div className={styles.title}>
                    ...
                    <h3>{wrong ? (isUz ? "NOTO‘G‘RI" : "НЕВЕРНО") : (isUz ? "TO‘G‘RI!" : "ПРАВИЛЬНО!")}</h3>
                </div>

                {correctText && (
                    <div className={styles.correctAnswer}>
                        <h4>{isUz ? "To‘g‘ri javob:" : "Правильный ответ:"}</h4>
                        <p>{correctText}</p>
                    </div>
                )}

                <Button
                    value={wrong ? (isUz ? "QAYTA URINISH" : "ПОВТОРИТЬ") : (isUz ? "DAVOM ETISH" : "ПРОДОЛЖИТЬ")}
                    className={wrong ? "red" : "green"}
                    onClick={wrong ? onRepeat : onClick}
                />
            </div>
        </>
    );
