...
const TypeOne = React.memo(() => {
  ...
  const getGameData = async (order) => {
    ...
    try {
      ...
      console.error(err);
      setError(isUz ? "Savolni yuklashda xatolik." : "Ошибка при загрузке вопроса.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.gameO}>
      {loading ? (
        ...
        <GameVideo
          url={"/assets/video/ending-1.0.mp4"}
          poster=""
          video=""
          onClick={() => {
            setVideoPlayed(true);
        ...
        <GameVideo
          url={"/assets/lessons/start-1-0.mp4"}
          poster=""
          video=""
        ...
        <>
          ...
          {alertHandler && (
            <Alert
              wrong={!isCorrect}
              correctText={!isCorrect && (isUz ? "Qayta urinib ko’ring" : "Попробуйте еще раз")}
