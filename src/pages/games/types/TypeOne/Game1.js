...
const TypeTwo = React.memo(() => {
  ...
  const getGameData = async (order) => {
    ...
    try {
      ...
      setError(isUz ? "So‘zni yuklashda xatolik." : "Ошибка при загрузке слова.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.game1}>
      {loading ? (
        ...
        <GameVideo
          url={"/assets/lessons/start-1-1.mp4"}
          poster=""
          video=""
        ...
        <>
          <div className={`${styles.Type2} heightScroll`}>
            <div className={styles.picture} onClick={handleChestClick}>
              <img
                ...
                alt="chest"
                style={{ opacity: chestOpen ? 0 : 1 }}
              />
              <img
                src="/assets/images/chest-open.png"
                alt="chest open"
                style={{ opacity: chestOpen ? 1 : 0 }}
              />
            </div>

            {chestOpen ? (
              <>
                <div className={styles.titleText}>{chestWord?.word}</div>
                <Title
                  text={"Повторите слово или фразу в микрофон"}
                />
              </>
            ) : (
              <p className={styles.openChest}>
                {isUz ? "Sandiqni oching" : "Откройте сундук"}
              </p>
            )}
          ...
          {alertHandler && (
            <Alert
              wrong={!isCorrect}
              correctText={
                !isCorrect && (isUz ? "Qayta urinib ko‘ring" : "Попробуйте еще раз")
              }
