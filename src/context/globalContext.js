...
export const GlobalProvider = ({ children }) => {
  ...
  const handleSpeak = (text) => {
    ...
    const selectedVoice =
      voices.find((voice) => voice.lang === "ru-RU") || voices[0];
    if (selectedVoice) {
  ...
  useEffect(() => {
  if (isCorrect !== true || !currentLessonId || !currentGameId) return;
  ...
  // Game ID ni localStorage'ga qo‘shish
  if (!storedData.includes(gameId)) {
    const updated = [...storedData, gameId];
    localStorage.setItem(moduleKey, JSON.stringify(updated));
  }

  // Barcha kerakli game IDlar mavjudligini tekshirish
  const updatedData = JSON.parse(localStorage.getItem(moduleKey)) || [];
  const expectedGames = moduleGameMap[currentLessonId];

  const isModuleComplete = expectedGames?.every((id) => updatedData.includes(id));

  if (isModuleComplete && !lessonBadge) {
    setLessonBadge(true);
    setFinishedLessonId(currentLessonId);
    localStorage.removeItem(moduleKey); // optional
  }
}, [isCorrect]);
