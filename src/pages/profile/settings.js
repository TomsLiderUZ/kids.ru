...
const Settings = () => {
  ...
  const { isUz, setIsUz } = useGlobalContext()
  ...
  return (
    <div className={styles.Settings}>
      <div className={styles.picture}>
        <div className={styles.name}>{isUz ? "Sozlamalar" : "НАСТРОЙКИ"}</div>
      </div>
      <div className={styles.settingsButtons}>
        ...
        <div className={styles.settingsItem}>
          ...
          <div className={`${styles.switchButton} ${isUz ? styles.switchLang : ""}`} onClick={toggleLanguage}>
            <div className={styles.switchIndicator}>{isUz ? "UZ" : "RU"}</div>
          </div>
        </div>
      </div>
      ...
      <div className={styles.support}>
        ...
        <div className={styles.buttons}>
          <button className={styles.editButton}>
            <img src="/assets/images/flag.png" alt="" /> +998 (99) 999-99-99
          </button>
          <button style={{ backgroundColor: "var(--main_components_primary)" }} className={styles.settingsButton}>
      ...
      <div className={`${styles.card} ${styles.streakCard} ${styles.aqua}`}>
        <div className={styles.label}>
          <span>{isUz ? "Ulashish" : "Поделиться"}</span>
        </div>
